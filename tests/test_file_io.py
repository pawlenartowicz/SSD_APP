...
class TestCreateProjectStructure:
    def test_creates_all_dirs(self, tmp_path):
        ...
        assert (proj_path / "embeddings").is_dir()
...
class TestEmbeddingHash:
    ...
    def test_find_duplicate(self, tmp_path):
        ...
        existing = emb_dir / "glove.ssdembed"
    ...
    def test_find_no_duplicate(self, tmp_path):
        project = _make_project(tmp_path)
        ProjectIO.create_project_structure(project.project_path)
        emb_dir = project.project_path / "embeddings"
...
class TestListPreparedEmbeddings:
    ...
    def test_lists_ssdembed_files(self, tmp_path):
        ...
        (emb_dir / "glove.ssdembed").write_bytes(b"not a real pickle")
        (emb_dir / "nkjp.ssdembed").write_bytes(b"also not a pickle")
        # Non-.ssdembed files should be ignored
        (emb_dir / "readme.txt").write_text("ignore me")
    ...
    def test_ignores_vectors_npy_sidecar(self, tmp_path):
        """*.vectors.npy sidecars must not appear as separate entries."""
        project = _make_project(tmp_path)
        ProjectIO.create_project_structure(project.project_path)
        emb_dir = project.project_path / "embeddings"

        (emb_dir / "glove.ssdembed").write_bytes(b"fake")
        (emb_dir / "glove.ssdembed.vectors.npy").write_bytes(b"sidecar data")
...
class TestErrorHandling:
    ...
    def test_load_result_corrupted_pkl(self, tmp_path):
        """Corrupted results.pkl → error status, not crash."""
        project = _make_project(tmp_path)
        ProjectIO.create_project_structure(project.project_path)
        result = _make_result(project, status="complete")
